#!/usr/bin/env python3
"""Visualize POASTA dynamic programming matrices alongside the graph.

The module provides a CLI that renders individual alignment iterations
as heatmaps with the accompanying POASTA graph layout.  Animations can
also be exported if ``matplotlib`` and ``ffmpeg`` are installed.
"""
import argparse
import functools
import re
...
def load_spoa_matrix(fname):
    """Load a SPOA DP matrix from a TSV file.

    The TSV is expected to be formatted as generated by the ``spoa``
    aligner.  The resulting data frame contains the DP scores as well
    as coordinate labels for plotting.

    Args:
        fname: Path to the TSV file.

    Returns:
        Tuple of ``x`` labels, ``y`` labels and a ``pandas.DataFrame``
        describing the matrix content.
    """
        ...
        for i, line in enumerate(f):
            ...
            if i == 0:
                ...
                for col, score in enumerate(parts[2:]):
                    ...
                    col_data.append(col)
                    score_data.append(
                        int(score) if score != "2147482624" else float("NaN")
                    )

    return (
        xlabels,
        ylabels,
        pandas.DataFrame({"rank": row_data, "offset": col_data, "score": score_data}),
    )
...
def poa_graph_layout(g):
    """Create a 2D layout for a POASTA graph using GraphViz.

    The graph is arranged horizontally according to GraphViz while the
    vertical position derives from the precomputed node rank.

    Args:
        g: The ``networkx`` graph representing the POASTA alignment.
    ...
    new_pos = {n: (pos[n][0], ndata["rank"] + 0.5) for n, ndata in g.nodes(data=True)}

    return new_pos


def poa_matrix_discontinuieties(g):
    """Identify discontinuities in the POA DP matrix.

    Horizontal lines in the heatmap correspond to branches or merges in
    the graph.  These discontinuities are useful for annotating the DP
    matrix plot.
    ...
    Returns:
        Sorted list of matrix row indices where discontinuities occur.
    """
...
def load_astar_data(fname):
    """Read A* search iteration data from a TSV.

    Args:
        fname: Path to the TSV produced by ``poasta`` during alignment.

    Returns:
        Tuple of ``pandas.DataFrame`` with iteration metrics and a dict of
        key-value metadata extracted from the file header.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot POASTA aligner computation state"
    )

    parser.add_argument(
        "graph", type=Path, help="The graph used for alignment in DOT format"
    )
    parser.add_argument(
        "astar_data_tsvs",
        type=Path,
    ...
    sorted_astar_tsvs: list[Path] = list(
        sorted(args.astar_data_tsvs, key=lambda v: astar_iters[v.name])
    )
...
def create_animation(
    g, graph_layout_data, sequence, all_astar_df, output_dir, prefix, fig_width=None
):
    ...
    _writer = FFMpegWriter(fps=10, codec="libwebp")
    ...
    for row, kind in enumerate(["match", "deletion", "insertion"]):
        fig, axes = plt.subplots(
            ...
            width_ratios=[1, 8.75, 0.25],
            constrained_layout=True,
        )

        init_func = functools.partial(
            draw_graph, g=g, graph_layout_data=graph_layout_data, ax=axes[0]
        )
...
def draw_graph(g, graph_layout_data, ax):
    nx.draw(
        g,
        pos=graph_layout_data.node_pos,
        ax=ax,
        node_size=75,
        node_color="white",
        linewidths=1,
...
def make_frame(
    ...
    seaborn.heatmap(
        ...
        vmax=max_score,
        xticklabels=xlabels,
        yticklabels=graph_layout_data.ylabels,
        annot=True,
        fmt="g",
